...
CREATE POLICY "crops_own" ON public.crops FOR ALL USING (
  EXISTS (SELECT 1 FROM public.farms WHERE id = crops.farm_id AND user_id = auth.uid())
);
...
CREATE POLICY "predictions_own" ON public.predictions FOR ALL USING (
  EXISTS (SELECT 1 FROM public.disease_reports WHERE id = predictions.report_id AND user_id = auth.uid())
);
